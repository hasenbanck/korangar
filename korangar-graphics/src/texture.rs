...
use crate::{BindlessSupport, ScreenSize};

static TEXTURE_ID: AtomicU64 = AtomicU64::new(0);

/// A GPU texture with an associated bind group.
pub struct Texture {
    id: u64,
    label: Option<String>,
    byte_size: usize,
...
impl Texture {
    /// Creates a new texture from a descriptor without initial data.
    ...
    /// Creates a new texture from a descriptor and uploads image data including
    /// mipmaps.
    ...
    /// Returns the 3D extent of the texture.
...
/// A collection of textures with optional bindless support.
#[derive(Debug)]
...
impl TextureSet {
    /// Creates a new texture set with optional bindless binding support.
    ...
    /// Returns the bind group for an individual texture by its ID.
    pub fn get_texture_bind_group(&self, texture_id: i32) -> &BindGroup {
        self.textures[texture_id as usize].get_bind_group()
    }

    /// Returns the bindless array bind group if available.
...
/// A cube map array texture.
pub(crate) struct CubeArrayTexture {
    label: Option<String>,
    texture: wgpu::Texture,
    texture_view: TextureView,
...
impl CubeArrayTexture {
    ...
    /// Returns the pixel format of the cube array texture.
    pub fn get_texture_format(&self) -> TextureFormat {
        self.texture.format()
    }

    /// Returns a reference to the full cube array texture view.
    pub fn get_texture_view(&self) -> &TextureView {
        &self.texture_view
    }

    /// Returns a reference to a specific cube face view.
...
/// A texture used as a render target attachment.
pub(crate) struct AttachmentTexture {
    label: Option<String>,
    texture: wgpu::Texture,
    texture_view: TextureView,
...
impl AttachmentTexture {
    /// Creates a new attachment texture with optional width padding.
    ...
    /// Returns a reference to a specific array layer's texture view.
    ...
    /// Returns a bind group layout for attachment textures with specified
    /// parameters.
...
/// A texture that can be used for storage operations in compute shaders.
...
impl StorageTexture {
    /// Creates a new storage texture with the specified dimensions and format.
    ...
    /// Returns a reference to the storage texture view.
    pub fn get_texture_view(&self) -> &TextureView {
        &self.texture_view
    }
}
