...
use crate::{Msaa, RENDER_TO_TEXTURE_DEPTH_FORMAT, RENDER_TO_TEXTURE_FORMAT};

/// Maximum number of binding array elements per shader stage.
pub(crate) const MAX_BINDING_ARRAY_ELEMENTS_PER_SHADER_STAGE: u32 = 10240;

/// Maximum size of storage buffer bindings in bytes.
pub(crate) const MAX_STORAGE_BUFFER_BINDING_SIZE: u32 = 268435456;

/// The maximum texture size that is guaranteed by the graphic engine to be
/// available.
pub const MAX_TEXTURE_SIZE: u32 = 8192;

/// The level of bindless texture support available.
...
/// Holds information about GPU capabilities and feature support.
...
impl Capabilities {
    /// Creates capabilities from the given adapter.
    ...
    /// Returns the supported MSAA levels for this GPU.
    pub fn get_supported_msaa(&self) -> &[Msaa] {
        self.supported_msaa.as_ref()
    }

    /// Returns the required WGPU features for the graphics engine.
    pub fn get_required_features(&self) -> Features {
        self.required_features
    }

    /// Returns the required WGPU limits for the graphics engine.
