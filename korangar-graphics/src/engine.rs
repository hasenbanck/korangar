...
use crate::{NUMBER_OF_POINT_LIGHTS_WITH_SHADOWS, ScreenSize};

/// Descriptor for initializing a graphics engine.
pub struct GraphicsEngineDescriptor {
    /// GPU capabilities and limits.
    pub capabilities: Capabilities,
    /// WGPU instance for GPU access.
    ...
    /// Atomic value for picking results.
    pub picker_value: Arc<AtomicU64>,
    /// Results of the last shadow cascade partition data for the directional
    /// light.
    pub directional_shadow_partitions: Arc<Mutex<[DirectionalShadowPartition; PARTITION_COUNT]>>,
}

/// Main graphics engine managing rendering pipeline and resources.
///
/// Bind Group layout:
///
...
impl GraphicsEngine {
    /// Initializes a new graphics engine with the provided descriptor.
    ...
    /// Resumes graphics engine after suspension, creating or reconfiguring
    /// surface and contexts.
    ...
    pub fn on_resume(
        ...
        if self.surface.is_none() {
            ...
            if self.previous_surface_texture_format != Some(surface_texture_format) {
                ...
                let interface_render_pass_context = InterfaceRenderPassContext::new(&self.device, &self.queue, &global_context);
                let picker_render_pass_context = PickerRenderPassContext::new(&self.device, &self.queue, &global_context);
                let directional_shadow_pass_context = DirectionalShadowRenderPassContext::new(&self.device, &self.queue, &global_context);
                let point_shadow_pass_context = PointShadowRenderPassContext::new(&self.device, &self.queue, &global_context);
                let light_culling_pass_context = LightCullingPassContext::new(&self.device, &self.queue, &global_context);
                let forward_pass_context = ForwardRenderPassContext::new(&self.device, &self.queue, &global_context);
                let sdsm_pass_context = SdsmPassContext::new(&self.device, &self.queue, &global_context);
                let post_processing_pass_context = PostProcessingRenderPassContext::new(&self.device, &self.queue, &global_context);
                let screen_blit_pass_context = ScreenBlitRenderPassContext::new(&self.device, &self.queue, &global_context);
    ...
    /// Returns the current bindless resource support level.
    #[cfg(feature = "debug")]
    ...
    /// Needs to be called on suspended. Releases the surface on Android
    /// platforms.
    ...
    /// Updates the surface dimensions when the window is resized.
    ...
    /// Sets the framerate limit for the rendering loop.
    ...
    /// Enables or disables triple buffering for the swap chain.
    ...
    /// Updates the texture sampling filter type.
    ...
    /// Sets the screen-space anti-aliasing method.
    ...
    /// Updates the multisample anti-aliasing sample count.
    ...
    /// Updates the supersample anti-aliasing factor.
    ...
    /// Updates the resolution of shadow maps.
    ...
    /// Enables or disables high-quality interface rendering.
    ...
    /// Returns the name of the graphics backend in use.
    pub fn get_backend_name(&self) -> String {
        self.adapter.get_info().backend.to_string()
    }

    /// Returns which presentation modes are supported.
    pub fn get_present_mode_info(&self) -> PresentModeInfo {
        self.surface.as_ref().unwrap().present_mode_info()
    }

    /// Returns the list of supported MSAA sample counts.
    ...
    /// Waits for the next frame and returns the surface texture to render to.
    ...
    /// Renders the next frame using the provided surface texture and render
    /// instructions.
    #[cfg_attr(feature = "debug", korangar_debug::profile)]
