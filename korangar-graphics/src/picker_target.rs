...
pub enum PickerTarget {
    ...
    Nothing,
    /// A map tile was picked at the specified coordinates.
    Tile {
    ...
    /// A game entity was picked with the given entity ID.
    Entity(u32),
    /// A debug marker was picked.
    #[cfg(feature = "debug")]
    Marker(MarkerIdentifier),
}

impl PickerTarget {
    /// Returns the size in bytes of an encoded picker target value.
    pub(crate) const fn value_size() -> usize {
        size_of::<u64>()
    }
}

/// Decodes a picker target from a u64 value.
impl From<u64> for PickerTarget {
    fn from(data: u64) -> Self {
        ...
        if data >> 32 == PickerValueType::Entity as u64 {
            return Self::Entity(data as u32);
        }
...
/// Encodes a picker target as a u64 value.
...
/// Encodes a picker target as a tuple of high and low 32-bit values.
impl From<PickerTarget> for (u32, u32) {
    fn from(picker_target: PickerTarget) -> Self {
        match picker_target {
            ...
            PickerTarget::Entity(entity_id) => (PickerValueType::Entity as u32, entity_id),
            #[cfg(feature = "debug")]
...
mod encoding {
    ...
    fn from_u64() {
        ...
        assert_eq!(((u64::from(high)) << 32) | u64::from(low), encoded);
    }
