...
/// A timestamp type that can be compared to calculate time differences.
pub(crate) trait ComparativeTimestamp: Copy {
...
/// Represents a stage in the rendering pipeline with timing information.
#[derive(Debug, Copy, Clone, PartialEq, Eq)]
pub(crate) struct FrameStage<T: ComparativeTimestamp> {
    index: usize,
    base: T,
}

/// Manages frame pacing to synchronize rendering with refresh rate.
...
impl FramePacer {
    /// Creates a new frame pacer with the specified refresh rate.
    ...
    /// Creates a new frame stage for tracking.
    ...
    /// Marks the end of a frame stage at the given timestamp.
    ...
    /// Sleeps until the next optimal frame time to maintain target framerate.
