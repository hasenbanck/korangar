//! This crate implements the graphics engine for the client.
...
pub(crate) use self::frame_pacer::*;
pub use self::instruction::*;
#[cfg(feature = "debug")]
pub use self::marker_identifier::MarkerIdentifier;
...
use crate::sampler::{SamplerType, create_new_sampler};

/// The number of points with lights that are supported by the graphics engine.
// TODO: The number of point lights that can cast shadows should be configurable
//       through the graphics settings. For now I just chose an arbitrary
//       smaller number that should be playable on most devices.
pub const NUMBER_OF_POINT_LIGHTS_WITH_SHADOWS: usize = 6;
...
/// We can't make this an overridable constant in WGPU or runtime defined,
/// since we use the const in the shaders in locations that need const and
/// don't allow overrides. So if you change this variable, you also need to
/// replace the constants in the shaders of the same name.
pub const PARTITION_COUNT: usize = 3;

pub(crate) const RENDER_TO_TEXTURE_FORMAT: TextureFormat = TextureFormat::Rgba8UnormSrgb;
pub(crate) const RENDER_TO_TEXTURE_DEPTH_FORMAT: TextureFormat = TextureFormat::Depth32Float;
pub(crate) const INTERFACE_TEXTURE_FORMAT: TextureFormat = TextureFormat::Rgba8UnormSrgb;
pub(crate) const FXAA_COLOR_LUMA_TEXTURE_FORMAT: TextureFormat = TextureFormat::Rgba8UnormSrgb;
...
/// The view partitions of the directional shadows camera when using sampled
/// distribution shadow maps (SDSM).
#[derive(Copy, Clone, Debug)]
pub struct DirectionalShadowPartition {
    /// The extends of the partition cube.
    pub extents: Vector4<f32>,
    /// The center of the partition cube.
    pub center: Vector4<f32>,
    /// The Z value this partition end in the main camera's view space.
