...
use crate::{DirectionalShadowPartition, PARTITION_COUNT, Partition};
...
impl<T: Sized + Pod + Zeroable> Buffer<T> {
    ...
    /// Creates a new buffer and immediately writes the given data to it.
...
/// A GPU buffer for dynamic uniform data with alignment handling.
pub(crate) struct DynamicUniformBuffer<T> {
    buffer: Buffer<u8>,
    data: Vec<u8>,
    aligned_size: usize,
...
impl<T: Sized + Pod + Zeroable> DynamicUniformBuffer<T> {
    /// Creates a new dynamic uniform buffer with proper alignment.
    ...
    /// Writes uniform data to the CPU-side buffer with proper alignment.
    ...
    /// Uploads the CPU-side buffer data to the GPU.
    ...
    /// Returns the dynamic offset for the given index.
    pub fn dynamic_offset(&self, index: usize) -> u32 {
        (index * self.aligned_size) as u32
    }

    /// Returns the binding type for this dynamic uniform buffer.
    ...
    /// Returns the binding resource for this dynamic uniform buffer.
