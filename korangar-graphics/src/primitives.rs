...
impl ScreenPosition {
    /// Creates a position from a size, using width as left and height as top.
    pub fn from_size(ScreenSize { width, height }: ScreenSize) -> Self {
        Self { left: width, top: height }
    }

    /// Creates a position with only the left value set, top is zero.
    pub fn only_left(left: f32) -> Self {
        Self { left, top: 0.0 }
    }

    /// Creates a position with only the top value set, left is zero.
...
impl ScreenSize {
    /// Creates a size with only the width value set, height is zero.
...
/// Defines padding for drop shadows with separate values for each side.
#[derive(Debug, Clone, Copy, Default, PartialEq, Serialize, Deserialize)]
...
impl ShadowPadding {
    /// Creates shadow padding with the same value for all sides.
    pub fn uniform(value: f32) -> Self {
        Self {
            left: value,
            right: value,
...
/// Defines the diameter of rounded corners for each corner of a rectangle.
#[derive(Debug, Clone, Copy, Default, PartialEq, Serialize, Deserialize)]
pub struct CornerDiameter {
    /// Top-left corner diameter in pixels.
    pub top_left: f32,
    /// Top-right corner diameter in pixels.
    pub top_right: f32,
    /// Bottom-left corner diameter in pixels.
    pub bottom_left: f32,
    /// Bottom-right corner diameter in pixels.
    pub bottom_right: f32,
}

impl CornerDiameter {
    /// Creates corner diameters with the same value for all corners.
    pub fn uniform(value: f32) -> Self {
        Self {
            top_left: value,
            top_right: value,
