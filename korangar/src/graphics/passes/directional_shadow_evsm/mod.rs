...
pub(crate) use entity::{DirectionalShadowEvsmEntityDrawData, DirectionalShadowEvsmEntityDrawer};
pub(crate) use indicator::DirectionalShadowEvsmIndicatorDrawer;
pub(crate) use model::DirectionalShadowEvsmModelDrawer;
...
use wgpu::{
    ...
    Device, LoadOp, Operations, Queue, RenderPass, RenderPassColorAttachment, RenderPassDepthStencilAttachment, RenderPassDescriptor,
    ShaderStages, StoreOp, TextureFormat,
};
...
const PASS_NAME: &str = "directional shadow EVSM render pass";
...
pub(crate) struct DirectionalShadowEvsmRenderPassContext {
    uniforms_buffer: DynamicUniformBuffer<PassUniforms>,
    bind_group: BindGroup,
    evsm_moment_texture_format: TextureFormat,
...
impl RenderPassContext<{ BindGroupCount::Two }, { ColorAttachmentCount::One }, { DepthAttachmentCount::One }>
    for DirectionalShadowEvsmRenderPassContext
{
    ...
    fn new(device: &Device, _queue: &Queue, _texture_loader: &TextureLoader, global_context: &GlobalContext) -> Self {
        ...
        let evsm_moment_texture_format = global_context.directional_evsm_moment_texture.get_format();
        let directional_shadow_texture_format = global_context.directional_shadow_map_texture.get_format();

        Self {
            uniforms_buffer,
            bind_group,
            evsm_moment_texture_format,
    ...
    fn create_pass<'encoder>(
        ...
        let mut pass = encoder.begin_render_pass(&RenderPassDescriptor {
            ...
            color_attachments: &[Some(RenderPassColorAttachment {
                view: global_context.directional_evsm_moment_texture.get_array_texture_view(pass_data),
                resolve_target: None,
                ops: Operations {
    ...
    fn color_attachment_formats(&self) -> [TextureFormat; 1] {
        [self.evsm_moment_texture_format]
    }
...
impl Prepare for DirectionalShadowEvsmRenderPassContext {
...
impl DirectionalShadowEvsmRenderPassContext {
