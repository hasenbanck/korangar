...
use crate::graphics::passes::{
    BindGroupCount, ColorAttachmentCount, DepthAttachmentCount, DirectionalShadowEvsmRenderPassContext, Drawer, RenderPassContext,
};
...
const DRAWER_NAME: &str = "directional shadow EVSM entity";
...
pub(crate) struct DirectionalShadowEvsmEntityDrawData<'data> {
    pub(crate) partition_index: usize,
    pub(crate) instructions: &'data [EntityInstruction],
}

pub(crate) struct DirectionalShadowEvsmEntityDrawer {
...
impl Drawer<{ BindGroupCount::Two }, { ColorAttachmentCount::One }, { DepthAttachmentCount::One }> for DirectionalShadowEvsmEntityDrawer {
    type Context = DirectionalShadowEvsmRenderPassContext;
    type DrawData<'data> = DirectionalShadowEvsmEntityDrawData<'data>;
    ...
    fn new(
        ...
        let shader_module = match capabilities.bindless_support() {
            BindlessSupport::Full => shader_compiler.create_shader_module("directional_shadow", "entity_evsm_bindless"),
            _ => shader_compiler.create_shader_module("directional_shadow", "entity_evsm"),
        };
        ...
        let pipeline = device.create_render_pipeline(&RenderPipelineDescriptor {
            ...
            fragment: Some(FragmentState {
                ...
                compilation_options: PipelineCompilationOptions::default(),
                targets: &[Some(render_pass_context.color_attachment_formats()[0].into())],
            }),
            multiview: None,
...
impl Prepare for DirectionalShadowEvsmEntityDrawer {
...
impl DirectionalShadowEvsmEntityDrawer {
