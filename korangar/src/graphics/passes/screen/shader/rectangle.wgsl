...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    if instance.linear_filtering == 0 {
        return textureSample(textures[instance.texture_index - 1], nearest_sampler, input.texture_coordinates) * instance.color;
    } else {
        return textureSample(textures[instance.texture_index - 1], linear_sampler, input.texture_coordinates) * instance.color;
    }
}
