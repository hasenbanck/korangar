...
impl GlobalContext {
    ...
    fn create_screen_size_textures(device: &Device, screen_size: ScreenSize) -> ScreenSizeTextures {
        ...
        let screen_ms_factory = AttachmentTextureFactory::new(device, screen_size, 4);

        let diffuse_buffer_texture = screen_ms_factory.new_attachment(
            "diffuse buffer",
            TextureFormat::Rgba8UnormSrgb,
            AttachmentTextureType::ColorAttachment,
        );
        let normal_buffer_texture = screen_ms_factory.new_attachment(
            "normal buffer",
            TextureFormat::Rgba16Float,
            AttachmentTextureType::ColorAttachment,
        );
        let water_buffer_texture = screen_ms_factory.new_attachment(
            "water buffer",
            TextureFormat::Rgba8UnormSrgb,
            AttachmentTextureType::ColorAttachment,
        );
        let depth_buffer_texture = screen_ms_factory.new_attachment(
            "depth buffer",
            TextureFormat::Depth32Float,
            AttachmentTextureType::DepthAttachment,
        );
        let interface_buffer_texture = screen_ms_factory.new_attachment(
            "interface buffer",
            TextureFormat::Rgba8UnormSrgb,
            AttachmentTextureType::ColorAttachment,
        );
