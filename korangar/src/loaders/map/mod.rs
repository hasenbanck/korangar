...
use crate::loaders::{map_model_texture_to_texture_buffer, GameFileLoader, ModelLoader, TextureLoader};
...
impl MapLoader {
    pub fn load(
        ...
        let ground_vertex_count = ground_vertices.len() as u32;
        let ground_texture_mapping =
            map_model_texture_to_texture_buffer(texture_loader, &mut texture_cache, &mut texture_buffer, &ground_data.textures);

        let mut vertices = NativeModelVertex::to_vertices(ground_vertices, &ground_texture_mapping);
