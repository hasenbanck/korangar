...
impl TextureAtlas {
    ...
    fn maxrects_split(&self, free_rect: Rectangle, used_rect: Rectangle) -> (Option<Rectangle>, Option<Rectangle>) {
        let f_prime = if free_rect.max.x > used_rect.max.x {
            Some(Rectangle::new(Vector2::new(used_rect.max.x, free_rect.min.y), free_rect.max))
        } else {
            None
        };

        let f_double_prime = if free_rect.max.y > used_rect.max.y {
            Some(Rectangle::new(Vector2::new(free_rect.min.x, used_rect.max.y), free_rect.max))
        } else {
            None
        };
